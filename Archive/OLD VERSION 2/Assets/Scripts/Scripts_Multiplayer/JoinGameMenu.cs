...
public class JoinGameMenu : MonoBehaviour
{
	...
	CommentManager.CommentData selectedCommentData = new CommentManager.CommentData ();
	...
	{
		selectedGame = null;
		MasterServer.RequestHostList(MultiplayerManager.gameName);
	}
	...
	void OnGUI()
	{
		if (isEnabled)
		{
			...
			{
				GUI.Label (new Rect(startX3, startY, 200, 22), "Host Name: " + selectedCommentData.hostName);

				GUI.Label (new Rect(startX3, startY + 44, 200, 22), "Players In Game:");
				for (int i = 0; i < selectedCommentData.users.Count; i++)
				{
					GUI.Label (new Rect(startX3, startY + 66 + i * 22, 200, 22), "- " + selectedCommentData.users[i]);
				}

				GUI.Label (new Rect(startX3, endY - 200, 200, 22), "Protocol Version: " + selectedCommentData.protocol);
	...
	{
		selectedCommentData = CommentManager.GetCommentData(hostData.comment);
		passWord_required = selectedCommentData.passWord;
	}
	...
	private void JoinServer (HostData hostData)
	{
		...
		if (selectedCommentData.users.Contains(ProfileManager.acces.player.username))
		{
			error = "User already in game!";
		}
		if (selectedCommentData.protocol != MultiplayerManager.acces.protocolVersion)
		{
		...
		if (error == "")
		{
			StateManager.acces.StartLevel (selectedCommentData.gamemode, true, false, hostData);
		}
		else
		{
