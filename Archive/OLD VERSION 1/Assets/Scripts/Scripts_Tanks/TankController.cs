...
public class TankController : MonoBehaviour
{
	...
	protected void Initialise ()
	{
		...
		{
			networkView.RPC("SetMineList", RPCMode.AllBuffered, mineAmount);
		}
		else
		{
	...
	protected void FireShell (string shooter)
	{
		if (Time.time > nextFire && Time.timeScale != 0)
		{
			if (shells > 0 && !shotSpawn.gameObject.GetComponent <ShotSpawnProber>().isInObject)
			{
				...
				if (SwitchBox.isServerOn)
				{
					networkView.RPC("InstantiateShell", RPCMode.AllBuffered, shotSpawn.position, shotSpawn.rotation, shellMaxBounces, shellSpeed, shooter);
				}
				else
				{
	...
	protected void FireMine (string shooter, float delay)
	{
		if (Time.time > nextLay && Time.timeScale != 0)
		{
			...
			if (SwitchBox.isServerOn)
			{
				networkView.RPC("InstantiateMine", RPCMode.AllBuffered, transform.position, transform.rotation, mineRadius, shooter);
			}
			else
			{
